...
static void XOR(const unsigned char in1[8], const unsigned char in2[8], unsigned char out[8]);
static unsigned char *Bin2ASCII(const unsigned char byte[64], unsigned char bit[8]);
static unsigned char *ASCII2Bin(const unsigned char bit[8], unsigned char byte[64]);
static void GenSubKey(const unsigned char oldkey[8], unsigned char newkey[16][8]);
static void endes(const unsigned char m_bit[8], const unsigned char k_bit[8], unsigned char e_bit[8]);
static void undes(const unsigned char m_bit[8], const unsigned char k_bit[8], unsigned char e_bit[8]);
static void SReplace(unsigned char s_bit[8]);
...
{
    if((!in) || (!out) || (datalen<1) || (!key) || (!boEncrypt && ((datalen % 8) !=0)))
        return false;

    //原先的补位方式
    if (iPaddingType == ZERO_PADDING)
    {
        return CDesEnter(in, out, datalen, key, boEncrypt?0:1);
    }

    // pkcs5padding补位方式，将最后缺几位补几位对应的值，例如缺1位那么补位信息为0x01;
    //                                                       缺8位那么补位信息为0x08 0x08 0x08 ... 0x08总共8位
    unsigned char ucPaddingData[8] = {0};

    // 计算数据为8的倍数及需要补位长度
    int iCrcTimes = datalen / 8;
    int iPaddingDataLen = 8 - datalen % 8;
    int i = 0;

    if(boEncrypt) //选择的模式是加密
    {
        // 计算补位信息,先将补位信息全部按pkcs5padding方式填充
        memset(ucPaddingData, iPaddingDataLen, 8);

        if(datalen % 8 != 0) //将源数据填入
        {
            for(i = 0; i < (datalen % 8); i++)
            {
                ucPaddingData[i] = in[iCrcTimes*8 + i];
            }
        }

        // 将待加密字串分以8字节为单位分段加密
        for(i = 0; i < iCrcTimes; i++)
        {
            endes(in + i*8, key, out + i*8);
        }

        // 将最后的补位信息进行加密
        endes(ucPaddingData, key, out + iCrcTimes * 8);
    }
    else //选择的模式是解密
    {
        // 将密文以8字节为单位分段解密
        ...
        //将补位信息去掉
        memset(out+i-out[i-1], 0x00, out[i-1]);
    }
    return true;
}
...
*     无
*   参数：
*     const unsigned char in1[8] 输入字符串1
*     const unsigned char in2[8] 输入字符串2
*     unsigned char out[8] 输出的结果字符串
*/
...
*     转换后结果的指针
*   参数：
*     const unsigned char byte[64] 输入字符串
*     unsigned char bit[8] 输出的转换结果
*/
...
*     转换后结果的指针
*   参数：
*     const unsigned char bit[8] 输入字符串
*     unsigned char byte[64] 输出的转换结果
*/
...
*     无
*   参数：
*     const unsigned char oldkey[8] 输入密钥
*     unsigned char newkey[16][8] 输出的子密钥
*/
...
*     无
*   参数：
*     const unsigned char m_bit[8] 输入的原文
*     const unsigned char k_bit[8] 输入的密钥
*     unsigned char e_bit[8] 输出的密文
*/
...
*     无
*   参数：
*     const unsigned char m_bit[8] 输入的密文
*     const unsigned char k_bit[8] 输入的密钥
*     unsigned char e_bit[8] 输出解密后的原文
*/
