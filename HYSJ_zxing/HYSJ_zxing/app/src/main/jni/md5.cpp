...
static void MD5_memcpy(unsigned char *, const unsigned char *, unsigned int);
...
/************************************************************
 *    desc:    use MD5 to crypt a string into cryptograph.
 *    rett:    void.
 *    para:
 *        ucpSrc:    pointer to the SourceData to crypt.
 *        nLenSrc:    the length of the SourceData.
 *        ucpDst:    pointer to the cryptograph data.
 *    memo:    ucpDst buffer must have more then 16Bytes space.
 ************************************************************/
void Md5Crypt(const unsigned char *ucpSrc, int nLenSrc, unsigned char *ucpDst)
{
    MD5_CTX    lrCtx;

    MD5Init( &lrCtx );
    MD5Update( &lrCtx, ucpSrc, nLenSrc );
    MD5Final( ucpDst, &lrCtx );
}
/*************** end of vMD5Crypt() *************************/

void Md5CryptStr(const unsigned char *ucpSrc, int nLenSrc, unsigned char *ucpDst)
{
    //const char *hex = "0123456789abcdef";
    const char *hex = "0123456789ABCDEF";
    MD5_CTX    lrCtx;
    int i = 0;
    unsigned char sTmp[33] = {0};
    unsigned char *r = ucpDst;
